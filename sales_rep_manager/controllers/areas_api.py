...
class AreasAPI(http.Controller):
    ...
    def _serialize_state(self, st, selected=False):
        return {
            "id": st.id,
            "name": noneify(st.name),
            "code": noneify(st.code),
            "country": {
    ...
    @http.route(
        ['/sales_rep_manager/<string:api_version>/governorates'],
        type='http', auth='none', methods=['GET', 'OPTIONS'], csrf=False, cors='*'
    )
    @jwt_required()
    def get_governorates(self, **kwargs):
        # CORS preflight
        ...
        try:
            ...
            return format_response(
                True, "Areas fetched successfully",
                {
                    "governorate": {
                        "id": country.id,
                        "name": noneify(country.name),
                        "code": noneify(country.code),
                    },
                    "total": total,
                    "count": len(states),
                    "limit": limit,
