        ...
        try:
            ...
            if mobile_invoice_number:
                ...
                if existing_credit:
                    ...
                    # ✅ استخراج الدفعات الفعلية المسجلة
                    existing_payments = env['account.payment'].sudo().search([
                        ('reconciled_invoice_ids', 'in', [existing_credit.id])
                    ])
                    payments_details = [{
                        "amount": p.amount,
                    ...
                    resp = {
                        ...
                        "payments_details": payments_details,  # ✅ الدفعات الحقيقية
            ...
            # ✅ تحذير إذا لم ترسل دفعات
            if not payment_list:
                _logger.warning(f">>>>>moh>>>> No payments provided for return {mobile_invoice_number}")
            ...
            with env.cr.savepoint():
                ...
                rep_profile = RepProfile.search([('user_id', '=', current_user.id)], limit=1)
                if not rep_profile:
                    raise UserError(f"No sales rep profile found for user {current_user.name} (ID: {current_user.id})")
                if not rep_profile.location_id:
                    raise UserError(
                        f"Sales rep profile exists but has no location assigned (Profile ID: {rep_profile.id})")
                ...
                for it in items:
                    ...
                    if is_damaged:
                        try:
                            ...
                            _logger.warning(
                                f">>>>>moh>>>> Failed to get product info for damaged item {product_id}: {str(e)}")
                ...
                # ✅ التحقق من نجاح الـ posting
                if credit.state != 'posted':
                    raise UserError(f"Failed to post credit note. Current state: {credit.state}")
                ...
                if incoming_msl_results and isinstance(incoming_msl_results, list):
                    try:
                        ...
                        _logger.warning(f">>>>>moh>>>> Failed to save MSL results: {str(e)}")

                # 6. الدفع (Payment)
                error_stage = 'payment'
                _logger.info(">>>>>moh>>>> Registering Payments (Return)")
                payments_info = []
                pay_date = None  # ✅ تعريف المتغير في البداية لحل المشكلة
                ...
                for pay in payment_list:
                    ...
                    if not pay_currency:
                        _logger.warning(f">>>>>moh>>>> Cannot resolve currency for payment: {pay}")
                        continue
                ...
                # ✅ في حالة عدم وجود دفعات، استخدم تاريخ الفاتورة
                if pay_date is None:
                    pay_date = credit.invoice_date or credit.date

                if final_note:
                    try:
                        picking.write({'note': final_note})
                    except Exception as e:
                        _logger.warning(f">>>>>moh>>>> Failed to update picking note: {str(e)}")
