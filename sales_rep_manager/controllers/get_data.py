...
class GetData(http.Controller):
    ...
    def get_customer_sales_by_category(self, partner_id, **kwargs):
        ...
        try:
            ...
            # ============================================================
            # البحث في سطور أوامر البيع المؤكدة بدلاً من الفواتير
            # لضمان الحصول على الكميات الصحيحة
            # ============================================================
            ...
            if date_to:
                # نضيف وقت نهاية اليوم لتشمل اليوم كاملاً
                from datetime import datetime, time
                date_to_end = datetime.combine(date_to.date(), time.max)
                domain.append(('order_id.date_order', '<=', date_to_end))
            if q:
                domain += [
                    '|',
            ...
            for line in lines:
                ...
                categ = prod.categ_id
                categ_name = categ.name if categ else "غير مصنّف"
                categ_id = categ.id if categ else 0

                # تهيئة الفئة
                if categ_id not in groups:
                    groups[categ_id] = {
                ...
                # تجميع المنتج داخل الفئة
                prod_id = prod.id
                qty = float(line.product_uom_qty or 0.0)
                subtotal = float(line.price_subtotal or 0.0)

                if prod_id in groups[categ_id]["products_dict"]:
                    groups[categ_id]["products_dict"][prod_id]["quantity"] += qty
                    groups[categ_id]["products_dict"][prod_id]["subtotal"] += subtotal
                else:
                    groups[categ_id]["products_dict"][prod_id] = {
                        "product_id": prod_id,
                ...
                groups[categ_id]["subtotal_amount"] += subtotal
            ...
            # ============================================================
            final_grouped_data = []
            for categ_id, data in groups.items():
                products_list = list(data["products_dict"].values())
                # ترتيب المنتجات من الأكثر مبيعاً
                products_list.sort(key=lambda x: x["quantity"], reverse=True)

                final_grouped_data.append({
                    "category": noneify(data["category_name"]),
                    "products": products_list,
                    "subtotal_wo_tax": round(data["subtotal_amount"], 2),
                })

            # ترتيب الفئات حسب إجمالي المبيعات
            final_grouped_data.sort(key=lambda x: x["subtotal_wo_tax"], reverse=True)
            ...
            return format_response(True, "Customer sales fetched successfully", {
                ...
                "groups": final_grouped_data,
                "grand_total_wo_tax": round(grand_total, 2)
            }, http_status=200)
